...
async (page) => {
  ...
  async function collectCandidates(workerPage) {
    return await workerPage.evaluate(() => {
      ...
      return {
        ...
        candidates: unique.slice(0, 16),
      }
    })
  }

  async function tryFetchCandidate(workerPage, candidates, rounds) {
    let lastResult = { ok: false }

    for (let index = 0; index < rounds; index += 1) {
      lastResult = await workerPage.evaluate(async (items) => {
  ...
  async function openSubtitles(workerPage) {
    await workerPage.keyboard.press('c').catch(() => {})
    await workerPage.waitForTimeout(1200)
  }

  async function hardRetry(workerPage) {
    ...
    await workerPage.waitForTimeout(4500)
    await openSubtitles(workerPage)
  ...
  async function processVideo(workerPage, bridgePage, video, workerIndex) {
    ...
    try {
      ...
      await workerPage.waitForTimeout(3500)

      let meta = await collectCandidates(workerPage)
      let fetched = await tryFetchCandidate(workerPage, meta.candidates, 2)

      if (!fetched.ok) {
        await openSubtitles(workerPage)
        meta = await collectCandidates(workerPage)
        fetched = await tryFetchCandidate(workerPage, meta.candidates, 2)
      }

      if (!fetched.ok) {
        await hardRetry(workerPage)
        meta = await collectCandidates(workerPage)
        fetched = await tryFetchCandidate(workerPage, meta.candidates, 2)
      }
